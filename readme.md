...
## 📱 Building & Compiling the Staff App to Android APK

The `staff-app` (`apps/staff-app`) is an Expo / React Native project configured with package identifier `com.hotelqr.staffapp` and `eas.json` for direct APK generation.

### 🌟 Method 1: Expo EAS Cloud Build (Recommended — No Android Studio/SDK Needed)

Builds the `.apk` on Expo's build servers and provides a direct download link and QR code:

1. Navigate to the `staff-app` folder:
   ```bash
   cd "apps/staff-app"
   ```

2. Log in to your Expo account (free at [expo.dev](https://expo.dev)):
   ```bash
   npx eas-cli login
   ```
...
4. When the build completes (approx. 3–5 minutes), open the provided download URL on your phone or computer to download `staff-app.apk`.

---

### 💻 Method 2: Local Gradle Build (Requires Android Studio & JDK 17+)

If you have **Android Studio** and the **Android SDK** configured locally on your computer:
...
3. Compile the release APK with Gradle:
   ```bash
   cd android
...
4. Output APK location:
   ```
   apps/staff-app/android/app/build/outputs/apk/release/app-release.apk
   ```

---

### ⚡ Method 3: Local EAS Build (Builds locally via EAS CLI)
