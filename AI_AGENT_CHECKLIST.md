...
- [x] Dynamic Theme & Content CMS (Migration 24):
  - Database schema: `packages/supabase/migrations/24_guest_web_theme_content_cms.sql` & `apps/web/supabase/migrations/24_guest_web_theme_content_cms.sql` — `theme_mode`, `theme_config` (JSONB hex palette), `content_config` (JSONB guest copy dictionary) on `hotels`.
  - Types: `GuestThemeMode`, `HotelThemeConfig`, `GuestWebContentConfig` (landing/dining/spa/requests/ai_chat/footer) exported from `packages/supabase/types/index.ts`.
  - Libraries: `apps/web/lib/guest-theme.ts` (5 presets: DARK_GOLD, CLEAN_LIGHT, MINIMAL_WHITE, LUXURY_NAVY, CUSTOM + `resolveGuestSurfaceTheme()` + WCAG `contrastRatio()`), `apps/web/lib/guest-content.ts` (`DEFAULT_CONTENT` + `mergeGuestContent()`).
  - Guest Web Provider: `apps/web/app/app/stay/components/GuestSettingsProvider.tsx` — fetches theme/content on mount, subscribes to Supabase Realtime, injects CSS vars (`--gw-bg`, `--gw-surface`, `--gw-text`, `--gw-text-2`, `--gw-accent`, `--gw-border`), exposes `useGuestContent()` hook.
  - Guest Web Pages Wired: `stay/page.tsx`, `WelcomeCardClient.tsx`, `dining/page.tsx`, `FnBDiningFAB.tsx`, `spa/page.tsx`, `requests/page.tsx`, `dining/checkout/page.tsx`, `CallFrontDeskModal.tsx`, `PhoneCaptureModal.tsx`.
  - Admin Branding Page: `/admin/branding` — 4 tabs (Visual Theme with preset swatches + custom color pickers + WCAG contrast, Welcome & Landing, Sections & Modules, Live Preview with phone mockup + JSON inspector), Publish button commits to Supabase.
  - Guest QR Resilience: `apps/web/app/app/stay/page.tsx` — split room validation from theme fetch, graceful fallback to DARK_GOLD if migration 24 columns missing.
- [ ] **PENDING: Supabase Account Migration** — Current project (`bsjnlawhdgfilcfejbji.supabase.co`) exhausted egress limits. Guide in `chat-history/2026-09-04_supabase-account-migration-guide.md`. Steps: export data, create new account/project, import data, apply migrations 23+24, update credentials in 6 files (apps/web/lib/supabase.ts, apps/staff-app/lib/supabase.ts, apps/staff-app/lib/foregroundService.ts, apps/web/lib/webPush.ts, apps/web/app/api/push/subscribe/route.ts, apps/web/app/api/push/webhook/route.ts), update Vercel env vars, OTA push staff app, verify QR flow.
